...
from create_crowdflower_hit_document_mod import create_crowdflower_document
...
def get_client(environment):
    ...
    client = boto.mturk.connection.MTurkConnection(
        ...
        host=get_boto2_parameters(environment)      # returns a requestor sandbox url()
    )
    return client
...
if __name__ == '__main__':
    ...
    if argument_length < 4:
        print("4 arguments required ....\n"
              "example: python script.py sandbox crowdflower <number_of_hits> <tweet_count_per_HIT>")
        sys.exit(0)
    ...
    if sys.argv[2] == 'crowdflower':
        ...
        with open(log_file_path, 'a+') as input_file:
            ...
            # go to the last line of log file to extract the last tweet posted/logged
