...
def insert_document(collection_name, final_collection, hit_id, start, tweet_count):
    ...
    for document in collection.find().skip(start).limit(tweet_count):
        # if i % 5 == 0:
        tweet_list.append({
        ...
        i += 1
	tweet_id_list.append(document['messageID'])
    count = i
    ...
    with open(get_log_directory('HITcreation') + 'hitID_Tweets_'  + get_timestamp() + '.txt', 'w') as log_tweetID:
        log_str = "HIT ID: " + hit_id + "\n" + "Tweets: " + str(tweet_id_list)
        log_tweetID.write(log_str)
